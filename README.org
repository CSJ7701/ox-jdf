...
** Fork
The only change I've made so far is to update the .cls file from the 2020 version to the most recent (2022).

/Notes:/
- You will need to install the .cls file manually. I did this with =cp ~/.emacs.d/straight/repos/ox-jdf/tex/latex/jdf ~/.texmf/tex/latex/=.
- The original author recommended using latexmk (later in this readme) for compilation. Footstomp on that. I tried pdflatex, which gave an error recommending xelatex, which I then tried. Xelatex produced some weird formatting errors, and I re-read the readme and switched to latexmk. My compilation process is defined as =(setq org-latex-pdf-process '("latexmk -f -pdf %f"))=.
- Using the above definition for =org-latex-pdf-process= will compile the latex, but not generate citations correctly. I found 2 options that seem to work for resolving this: =(setq org-latex-pdf-process '("pdflatex -shell-escape -interaction nonstopmode -output-directory %o %f" "biber --output-directory %o $(basename %f .tex)" "pdflatex -shell-escape -interaction nonstopmode -output-directory %o %f" "pdflatex -shell-escape -interaction nonstopmode -output-directory %o %f"))= OR =(setq org-latex-pdf-process '("latexmk -shell-escape -bibtex -f -pdf %f"))=. I am currently using the second.
